...
mongoose.connect('mongodb://localhost/studentData').then(()=>{
    console.log('Mongoose connected to Data Base');
}).catch((err)=>{
    console.log('Failed to connect...');
});
app.listen(3003,()=>{
    console.log('Server running at 3003 portal');
});
...
app.post('/api/student', (req, res) => {
    if (req.body.name && req.body.currentClass && req.body.division) {
      ...
      res.status(400).send('Failed check all your fields');
    }
  });
...
app.delete('/api/student/:id',(req,res)=>{
    let id= req.params.id
    bluePrint.findByIdAndDelete(id,req.body).then((data)=>{
